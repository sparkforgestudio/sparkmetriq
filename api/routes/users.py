...
from core.auths import get_current_user, is_admin
from core.permissions import has_role
from schemas.users import UserResponse, UserUpdate, UserPasswordUpdate, UserRole, UserRoleUpdate
...
@router.delete("/{email}")
async def delete_user(email: str, admin_user: UserResponse = Depends(has_role(UserRole.admin))):
...
async def update_user_role(email: str, role_update: UserRoleUpdate, admin_user: UserResponse = Depends(has_role(UserRole.admin))):
    if role_update.role not in UserRole.__members__.values():
        raise HTTPException(status_code=400, detail="Invalid role")

    result = await db["users"].update_one({"email": email}, {"$set": {"role": role_update.role}})
...
async def update_me(user_update: UserUpdate, current_user: UserResponse = Depends(get_current_user)):
    ...
    return UserResponse(email=updated_user["email"], role=updated_user["role"])

# 🔹 Changer son mot de passe
@router.put("/me/password")
async def change_password(password_data: UserPasswordUpdate, current_user: UserResponse = Depends(get_current_user)):
    ...
    await db["users"].update_one({"email": current_user.email}, {"$set": {"password": new_hashed_password}})
