# api/services/content_distributor/connectors/fansly.py
import httpx
import os
import json
import hashlib
...
# Variables d'environnement pour Fansly
FANSLY_API_KEY = os.getenv("FANSLY_API_KEY")
FANSLY_API_SECRET = os.getenv("FANSLY_API_SECRET")
FANSLY_BASE_URL = os.getenv("FANSLY_BASE_URL", "https://api.fansly.com/v1")
FANSLY_WEBHOOK_SECRET = os.getenv("FANSLY_WEBHOOK_SECRET")

class FanslyConnector:
    def __init__(self, api_key: str, api_secret: str):
        self.api_key = api_key
        self.api_secret = api_secret
        self.base_url = FANSLY_BASE_URL

    def _generate_signature(self, method: str, endpoint: str, timestamp: str, body: str = "") -> str:
        """Génère la signature HMAC pour l'authentification Fansly."""
    ...
    def _get_headers(self, method: str, endpoint: str, body: str = "") -> Dict[str, str]:
        """Génère les headers d'authentification pour Fansly."""
    ...
    async def get_profile_info(self) -> Dict[str, Any]:
        """Récupère les informations du profil Fansly."""
    ...
    async def create_post(self, content: Dict[str, Any]) -> Dict[str, Any]:
        ...
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code not in [200, 201]:
                raise Exception(f"Erreur création post Fansly: {data}")
            
            return data

    async def upload_media(self, media_url: str, media_type: str = "image") -> Dict[str, Any]:
        """
        Upload un média sur Fansly.
        ...
        async with httpx.AsyncClient() as client:
            ...
            data = response.json()
            if response.status_code not in [200, 201]:
                raise Exception(f"Erreur upload média Fansly: {data}")
            
            return data

    async def get_posts(self, limit: int = 20, offset: int = 0) -> Dict[str, Any]:
        """Récupère la liste des posts Fansly."""
        async with httpx.AsyncClient() as client:
            ...
            data = response.json()
            if response.status_code != 200:
                raise Exception(f"Erreur récupération posts Fansly: {data}")
            
            return data

    async def get_post_analytics(self, post_id: str, start_date: str, end_date: str) -> Dict[str, Any]:
        """Récupère les analytics d'un post Fansly."""
        async with httpx.AsyncClient() as client:
            ...
            data = response.json()
            if response.status_code != 200:
                raise Exception(f"Erreur analytics post Fansly: {data}")
            
            return data

    async def get_earnings(self, start_date: str, end_date: str) -> Dict[str, Any]:
        """Récupère les revenus Fansly pour une période."""
        async with httpx.AsyncClient() as client:
            ...
            data = response.json()
            if response.status_code != 200:
                raise Exception(f"Erreur récupération revenus Fansly: {data}")
    ...
    async def get_subscribers(self, limit: int = 50, offset: int = 0) -> Dict[str, Any]:
        ...
        async with httpx.AsyncClient() as client:
            ...
            data = response.json()
            if response.status_code != 200:
                raise Exception(f"Erreur récupération abonnés Fansly: {data}")
    ...
    async def create_tier(self, tier_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        async with httpx.AsyncClient() as client:
            ...
            data = response.json()
            if response.status_code not in [200, 201]:
                raise Exception(f"Erreur création tier Fansly: {data}")
            
            return data

    @staticmethod
    def verify_webhook_signature(payload: str, signature: str, secret: str) -> bool:
        """Vérifie la signature d'un webhook Fansly."""
...
@log_step
async def publish_to_fansly(content: Dict[str, Any], model_info: Dict[str, Any]) -> Dict[str, Any]:
    """
    Publie du contenu sur Fansly.

    :param content: Dictionnaire contenant title, description, media_urls, price, etc.
    :param model_info: Dictionnaire contenant fansly_api_key, fansly_api_secret, agency_id, muse_id.
    :return: Résultat de la publication.
    """
    try:
        api_key = model_info.get("fansly_api_key") or FANSLY_API_KEY
        api_secret = model_info.get("fansly_api_secret") or FANSLY_API_SECRET
        
        if not api_key or not api_secret:
            raise Exception("Clés API Fansly manquantes.")
        
        connector = FanslyConnector(api_key, api_secret)
        ...
        for media_url in media_urls:
            try:
                ...
                logger.warning(f"Erreur upload média Fansly {media_url}: {e}")
        ...
        await log_platform_event(
            ...
            status="success",
            message="Publication Fansly réussie",
            metadata={
                "title": title,
                "price": price,
                "is_premium": is_premium,
        ...
        await log_platform_event(
            ...
            content_id=content.get("id", ""),
            status="error",
            message=f"Erreur lors de la publication Fansly: {str(e)}",
            metadata={"error_type": type(e).__name__}
        )
        logger.error(f"Erreur publish_to_fansly: {e}")
        return {"status": "error", "reason": str(e)}

async def initialize_fansly_connector() -> None:
    """
    Fonction d'initialisation du connecteur Fansly.
    """
    if not FANSLY_API_KEY or not FANSLY_API_SECRET:
        logger.warning("Les variables d'environnement pour Fansly ne sont pas toutes définies.")
    else:
        logger.info("Connecteur Fansly initialisé avec succès.")
