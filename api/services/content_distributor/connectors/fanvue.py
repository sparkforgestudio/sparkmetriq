# api/services/content_distributor/connectors/fanvue.py
import httpx
import os
import json
import hashlib
...
# Variables d'environnement pour Fanvue
FANVUE_API_KEY = os.getenv("FANVUE_API_KEY")
FANVUE_API_SECRET = os.getenv("FANVUE_API_SECRET")
FANVUE_BASE_URL = os.getenv("FANVUE_BASE_URL", "https://api.fanvue.com/v1")
...
class FanvueConnector:
    def __init__(self, api_key: str, api_secret: str):
        ...
        self.base_url = FANVUE_BASE_URL

    def _generate_signature(self, method: str, endpoint: str, timestamp: str, body: str = "") -> str:
        """Génère la signature HMAC pour l'authentification Fanvue."""
    ...
    def _get_headers(self, method: str, endpoint: str, body: str = "") -> Dict[str, str]:
        """Génère les headers d'authentification pour Fanvue."""
    ...
    async def get_profile_info(self) -> Dict[str, Any]:
        """Récupère les informations du profil Fanvue."""
    ...
    async def create_post(self, content: Dict[str, Any]) -> Dict[str, Any]:
        ...
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code not in [200, 201]:
                raise Exception(f"Erreur création post Fanvue: {data}")
            
            return data

    async def upload_media(self, media_url: str, media_type: str = "image") -> Dict[str, Any]:
        """
        Upload un média sur Fanvue.
        ...
        async with httpx.AsyncClient() as client:
            ...
            data = response.json()
            if response.status_code not in [200, 201]:
                raise Exception(f"Erreur upload média Fanvue: {data}")
    ...
    async def get_posts(self, limit: int = 20, offset: int = 0) -> Dict[str, Any]:
        ...
        async with httpx.AsyncClient() as client:
            ...
            data = response.json()
            if response.status_code != 200:
                raise Exception(f"Erreur récupération posts Fanvue: {data}")
    ...
    async def get_post_analytics(self, post_id: str, start_date: str, end_date: str) -> Dict[str, Any]:
        ...
        async with httpx.AsyncClient() as client:
            ...
            data = response.json()
            if response.status_code != 200:
                raise Exception(f"Erreur analytics post Fanvue: {data}")
    ...
    async def get_earnings(self, start_date: str, end_date: str) -> Dict[str, Any]:
        ...
        async with httpx.AsyncClient() as client:
            ...
            data = response.json()
            if response.status_code != 200:
                raise Exception(f"Erreur récupération revenus Fanvue: {data}")
    ...
    async def get_subscribers(self, limit: int = 50, offset: int = 0) -> Dict[str, Any]:
        ...
        async with httpx.AsyncClient() as client:
            ...
            data = response.json()
            if response.status_code != 200:
                raise Exception(f"Erreur récupération abonnés Fanvue: {data}")
            
            return data

    @staticmethod
    def verify_webhook_signature(payload: str, signature: str, secret: str) -> bool:
        """Vérifie la signature d'un webhook Fanvue."""
...
@log_step
async def publish_to_fanvue(content: Dict[str, Any], model_info: Dict[str, Any]) -> Dict[str, Any]:
    """
    Publie du contenu sur Fanvue.

    :param content: Dictionnaire contenant title, description, media_urls, price, etc.
    :param model_info: Dictionnaire contenant fanvue_api_key, fanvue_api_secret, agency_id, muse_id.
    :return: Résultat de la publication.
    """
    try:
        api_key = model_info.get("fanvue_api_key") or FANVUE_API_KEY
        api_secret = model_info.get("fanvue_api_secret") or FANVUE_API_SECRET
        
        if not api_key or not api_secret:
            raise Exception("Clés API Fanvue manquantes.")
        
        connector = FanvueConnector(api_key, api_secret)
        ...
        for media_url in media_urls:
            try:
                ...
                logger.warning(f"Erreur upload média Fanvue {media_url}: {e}")
        ...
        await log_platform_event(
            ...
            status="success",
            message="Publication Fanvue réussie",
            metadata={
                "title": title,
                "price": price,
                "is_premium": is_premium,
        ...
        await log_platform_event(
            ...
            content_id=content.get("id", ""),
            status="error",
            message=f"Erreur lors de la publication Fanvue: {str(e)}",
            metadata={"error_type": type(e).__name__}
        )
        logger.error(f"Erreur publish_to_fanvue: {e}")
        return {"status": "error", "reason": str(e)}

async def initialize_fanvue_connector() -> None:
    """
    Fonction d'initialisation du connecteur Fanvue.
    """
    if not FANVUE_API_KEY or not FANVUE_API_SECRET:
        logger.warning("Les variables d'environnement pour Fanvue ne sont pas toutes définies.")
    else:
        logger.info("Connecteur Fanvue initialisé avec succès.")
